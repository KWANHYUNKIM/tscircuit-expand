...
| A4 | LM2576 레귤레이터 + 벅 서브회로 | med | ☑ | `to220_5`, `Buck5V`(LM+L+D+캡) (`LM2576.tsx`) |
| A5 | 릴레이 ALDP105 (커스텀 풋프린트) | med-high | ☐ | `.kicad_mod`/`<footprint>` platedhole |
| A6 | CN.1~19 다핀 커넥터 래퍼 | low | ☑ | `Conn`=`<pinheader>`+pinLabels배열 (`Connector.tsx`) |
...
| B2 | ERC 리포트 래퍼 | low | ☑ | `runErc`/`formatErc` (`expand/erc/runErc.ts`) |
